...
function StudentViewCoursesPage() {
  ...
  async function handleCourseNavigate(getCurrentCourseId) {
    ...
    console.log('Hello with current id');
    console.log(auth?.user?._id + '  user Id');
  ...
  return (
    <div className='container mx-auto p-4'>
      <h1 className='text-3xl font-bold mb-4 text-black'>All Courses</h1>
      <div className='flex flex-col md:flex-row gap-4'>
        <aside className='w-full md:w-64 space-y-4'>
          <div>
            {Object.keys(filterOptions).map(ketItem => (
              <div className='p-4 border-b'>
                <h3 className='text-gold font-bold'>{ketItem.toUpperCase()}</h3>
                <div className='grid gap-2 mt-2'>
                  {filterOptions[ketItem].map(option => (
                    <Label className='flex font-bold items-center gap-3 transform transition-all duration-300 hover:scale-105 hover:shadow-xl'>
                      <Checkbox
                        checked={
                          filters &&
                        ...
                        className="transform transition duration-300 ease-in-out scale-100 hover:scale-110 hover:shadow-xl hover:border-teal-600 focus:ring-teal-400 p-2 rounded-lg peer"
          ...
          <div className='flex justify-end items-center mb-4 gap-5'>
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button variant='outline' size='sm' className='bg-yellow-500 text-white font-bold text-[16px] hover:bg-teal hover:text-white'>
              ...
              studentViewCoursesList.map(courseItem => (
                <Card
                  onClick={() => handleCourseNavigate(courseItem?._id)}
                   className="cursor-pointer bg-white text-black p-6 rounded-xl hover:shadow-teal shadow-lg transform transition-all duration-300 ease-in-out hover:scale-105 hover:border-teal"
                  key={courseItem?._id}
                >
