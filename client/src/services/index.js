...
export async function adminStatsService() {
  const response = await axiosInstance.get('stats/statistics');
  console.log('ya hain stats: ', response);
  return response;
}
// export async function registerService(formData) {
//   try {
...
//     const {data} = await axiosInstance.post('/auth/register', {
//       userName: formData.username, // Match with backend
...
//     return data;
//   } catch (error) {
//     console.error('Error Response:', error.response?.data || error.message); // Log error from server
//     throw error;
//   }
// }
